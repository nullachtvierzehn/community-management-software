<template>
  <template v-if="!m">
    <div v-if="fetching" class="message message_loading">
      Die Nachricht lädt.
    </div>
    <form
    ...
    <div v-else class="message message_missing">
      Die Nachricht wurde nicht gefunden.
    </div>
  </template>
  <div v-if="m && !edit" class="message">
    <div v-if="m.sender" class="message__sender">
      {{ m.sender.username }}
    </div>
    <div v-else class="message__sender">Nutzer:in unbekannt</div>
